 ...
 * represented as a non-negative 2D map, in this map: 0 represents the obstacle
 * can't be reached. 1 represents the ground can be walked through. The place
 * with number bigger than 1 represents a tree can be walked through, and this
 * positive number represents the tree's height. In one step you can walk in any
 * of the four directions top, bottom, left and right also when standing in a
 * point which is a tree you can decide whether or not to cut off the tree. You
 ...
 * the original place has the tree will become a grass (value 1). You will start
 * from the point (0, 0) and you should output the minimum steps you need to
 * walk to cut off all the trees. If you can't cut off all the trees, output -1
 * in that situation. You are guaranteed that no two trees have the same height
...
public class CutOffTrees {
	 ...
	 * (int[] {row, col, height}) into a priority queue and sort by height. Poll
	 * each tree from the queue and use BFS to find out steps needed.
	 * 
