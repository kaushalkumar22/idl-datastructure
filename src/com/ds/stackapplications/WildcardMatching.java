package com.ds.stackapplications;
/**
...
public class WildcardMatching {
	...
	private static boolean wildcardMatchingLinearLime(String str, String pattern) {
		...
		{
			if (pIndex < pattern.length()  && (pattern.charAt(pIndex) == '?' 
					|| str.charAt(sIndex) == pattern.charAt(pIndex))){// advancing both pointers
				sIndex++;
				pIndex++;
			}else if (pIndex < pattern.length() && pattern.charAt(pIndex) == '*'){// * found, only advancing pattern pointer
				starIdx = pIndex;
				match = sIndex;
				pIndex++;
