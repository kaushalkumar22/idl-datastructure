 ...
 * the character key[i]. If the character key[i] has been aligned at the 12:00
 * direction, you need to press the center button to spell, which also counts as
 * 1 step. After the pressing, you could begin to spell the next character in
 * the key (next stage), otherwise, you've finished all the spelling. Example:
 * 
 * 
 * 
 * Input: ring = "godding", key = "gd" Output: 4 Explanation: For the first key
 * character 'g', since it is already in place, we just need 1 step to spell
 * this character. For the second key character 'd', we need to rotate the ring
 * "godding" anticlockwise by two steps to make it become "ddinggo". Also, we
 * need 1 more step for spelling. So the final output is 4. Note:
 * 
 *
 */
