...
public class LongestSubstringWithoutRepeatingCharacters {
	 ...
	 * defined by the start and end indices, i.e. [i,j)[i, j)[i,j) (left-closed,
	 * right-open). A sliding window is a window "slides" its two boundaries to the
	 * certain direction. For example, if we slide [i,j)[i, j)[i,j) to the right by
	 * 111 element, then it becomes [i+1,j+1)[i+1, j+1)[i+1,j+1) (left-closed,
	 * right-open). The reason is that if s[j]s[j]s[j] have a duplicate in the range
	 * [i,j)[i, j)[i,j) with index j j'j, we don't need to increase iii little by
