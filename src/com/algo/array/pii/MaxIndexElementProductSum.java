...
/**
 * Given an array, find the maximum possible value of sum of
 * index-element-products(i*array[i]) with only rotations allowed on a given
 * array. Sum of index-element-products for array of length 'n' is computed as -
 * 0*array[0] + 1*array[1] + 2*array[2] + ... + n-1*array[n-1].
 * 
 * For example, for the array {3,4,5,6,1,2} without doing any rotations sum of
 * index-element-products is 46. After doing one clockwise rotation of the
 * array, it would be modified to {2,3,4,5,6,1} and sum of
...
public class MaxIndexElementProductSum {
	...
	public int findMaxIndexElementProductSum(int[] array) {
		 ...
		 * the index-element-product sum after 'i' rotations would be currValue +
		 * sumElements - n*array[n-i]
		 */
		int maxValue = currValue, n = array.length;
