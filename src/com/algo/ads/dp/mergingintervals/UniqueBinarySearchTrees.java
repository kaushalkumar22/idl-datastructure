package com.algo.ads.dp.mergingintervals;
...
public class UniqueBinarySearchTrees {
	 ...
	 * number as the root, naturally, the subsequence 1…(i-1) on its left side would
	 * lay on the left branch of the root, and similarly the right subsequence
	 * (i+1)…n lay on the right branch of the root. We then can construct the
	 * subtree from the subsequence recursively. Through the above approach, we
	 * could ensure that the BST that we construct are all unique, since they have
	 * unique roots.
	 * 
	 ...
	 * 
	 * First of all, given the above definitions, we can see that the total number
	 * of unique BST G(n), is the sum of BST F(i) using each number i as a root.
	 * i.e.
	 * 
	 * G(n) = F(1, n) + F(2, n) + ... + F(n, n).
	 * 
	 * Particularly, the bottom cases, there is only one combination to construct a
	 * BST out of a sequence of length 1 (only a root) or 0 (empty tree). i.e.
	 * 
	 * G(0)=1, G(1)=1.
	 * 
	 * Given a sequence 1…n, we pick a number i out of the sequence as the root,
	 * then the number of unique BST with the specified root F(i), is the cartesian
	 ...
	 * left subsequence [1, 2] and another BST out of the right subsequence [4, 5,
	 * 6, 7], and then combine them together (i.e. cartesian product). The tricky
	 ...
	 * Example: Input: 3 Output: [ [1,null,3,2],
	 * [3,2,null,1], [3,1,null,null,2], [2,1,3], [1,null,2,null,3] ] 
